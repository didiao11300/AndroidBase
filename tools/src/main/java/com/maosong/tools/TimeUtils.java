...
public class TimeUtils {
    ...
    private static SimpleDateFormat getDefaultFormat1() {
        SimpleDateFormat simpleDateFormat = SDF_THREAD_LOCAL.get();
        if (simpleDateFormat == null) {
    ...
    public static String getEnglishFormat(final long millis){
        return new SimpleDateFormat("MM dd , yyyy").format(millis);
    }
    private TimeUtils() {
    ...
    public static String millis2String2(final long millis) {
        return millis2String(millis, getDefaultFormat1());
    }
    /**
     * Milliseconds to the formatted time string.
     *
    ...
    public static String getFitTimeSpan(final Date date1, final Date date2, final int precision, String[] units) {
        return millis2FitTimeSpan(date2Millis(date1) - date2Millis(date2), precision, units);
    }

    public static List<String> getFitTimeSpan2(final Date date1, final Date date2, final int precision) {
        return millis2FitTimeSpan2(date2Millis(date1) - date2Millis(date2), precision);
    }
    ...
    public static String getFitTimeSpanByNow(final Date date, final int precision, String[] units) {
        return getFitTimeSpan(date, getNowDate(), precision, units);
    }

    public static List<String> getFitTimeSpanByNow2(final Date date, final int precision) {
        return getFitTimeSpan2(date, getNowDate(), precision);
    }
    ...
    public static String getFriendlyTimeSpanByNow(final long millis) {
        ...
        if (span < 1000) {
            return ToolsApp.getAppContext().getString(R.string.gangang);
        } else if (span < TimeConstants.MIN) {
            return String.format(Locale.getDefault(), ToolsApp.getAppContext().getString(R.string.miao_before), span / TimeConstants.SEC);
        } else if (span < TimeConstants.HOUR) {
            return String.format(Locale.getDefault(), ToolsApp.getAppContext().getString(R.string.fen_before), span / TimeConstants.MIN);
        }
        // 获取当天 00:00
        long wee = getWeeOfToday();
        if (millis >= wee) {
            return String.format(ToolsApp.getAppContext().getString(R.string.today), millis);
        } else if (millis >= wee - TimeConstants.DAY) {
            return String.format(ToolsApp.getAppContext().getString(R.string.zuotian), millis);
        } else {
    ...
    /*将时间段转换成年*/
    public static long millis2TimeSpanYear(final long millis) {
    ...
    private static String millis2FitTimeSpan(long millis, int precision, String[] units) {
        if (precision <= 0) return null;
        precision = Math.min(precision, 5);
        if (millis == 0) return 0 + units[precision - 1];
        StringBuilder sb = new StringBuilder();
        ...
        for (int i = 0; i < precision; i++) {
            if (millis >= unitLen[i]) {
                long mode = millis / unitLen[i];
                millis -= mode * unitLen[i];
                sb.append(format.format(mode)).append(units[i]);
            } else {
                sb.append(format.format(0)).append(units[i]);
            }
        }
        return sb.toString();
    }
    ...
    private static List<String> millis2FitTimeSpan2(long millis, int precision) {
        if (precision <= 0) return null;
        precision = Math.min(precision, 5);
        ...
        for (int i = 0; i < precision; i++) {
            if (millis >= unitLen[i]) {
                long mode = millis / unitLen[i];
                millis -= mode * unitLen[i];
                result.add(format.format(mode));
            } else {
                result.add(format.format(0));
            }
        }
        return result;
    }
    ...
    public static String getNowCalendar(){
    ...
    public static long timeInMillisFormat(Calendar calendar, TextView view,TextView v, Context context) throws ParseException {
        showTimePickerDialog(context, v, calendar);
        showDatePickerDialog(context, 2, view, calendar);
        calendar.setTime(
                new SimpleDateFormat("dd/MM/yyyyHH:mm").parse(view.getText().toString() + v.getText().toString()));
    ...
    public static void showTimePickerDialog(Context context, final TextView tv, Calendar calendar) {
        // 创建一个TimePickerDialog实例，并把它显示出来
        new TimePickerDialog(context, TimePickerDialog.THEME_HOLO_DARK,
                // 绑定监听器
                (view, hourOfDay, minute) -> tv.setText(hourOfDay + ":" + minute)
                // 设置初始时间
