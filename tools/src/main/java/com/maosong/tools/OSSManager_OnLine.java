 ...
 * bucket: 'jfy2018'
 */
public class OSSManager_OnLine {

    private static OSSManager_OnLine mInstance;
    private OSSClient ossClient;
//    private final String region = "oss-cn-shenzhen.aliyuncs.com";
//    private final boolean secure = true;

    private OSSManager_OnLine(Context context) {
        init(context);
    }

    public static OSSManager_OnLine getInstance(Context context) {
        if (mInstance == null) {
            mInstance = new OSSManager_OnLine(context);
        }
        return mInstance;
    }
    ...
    public String upLoadVoiceSync(File file) throws ClientException, ServiceException {
        // 构造上传请求
        String bucket = "yt-bullet-s";
        // 构造上传请求
        PutObjectRequest put = new PutObjectRequest(bucket, "userVoice/" + file.getName(), file.getAbsolutePath());
        ossClient.putObject(put);
        return getFileUrl("userVoice/", file.getName());
    }
    ...
    public String upLoadVideoSync(File file) throws ClientException, ServiceException {
        // 构造上传请求
        String bucket = "yt-bullet-s";
        // 构造上传请求
        PutObjectRequest put = new PutObjectRequest(bucket, "video/" + file.getName(), file.getAbsolutePath());
        ossClient.putObject(put);
        return getFileUrl("video/", file.getName());
    }

    private String getFileUrl(String prex, String fileName) {
        String fileUrlFix = "https://yt-bullet-s.oss-ap-southeast-1.aliyuncs.com/" + prex;
