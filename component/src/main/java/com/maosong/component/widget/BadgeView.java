...
import com.maosong.component.activity.DeveloperOptionsActivity;

/**
 * Created by tory on 2018/6/28.
 */

public class BadgeView extends android.support.v7.widget.AppCompatTextView {

    private boolean mHideOnNull = true;
    public final int DEFAULT_HEIGHT_WIGHT = 12;//dp
...
//      setPadding(dip2Px(1), dip2Px(0.5f), dip2Px(1.5f), dip2Px(0.5f));
        setPadding(0, 0, 0, dip2Px(1f));

        // set default background
        ViewGroup.LayoutParams lp = getLayoutParams();
        int wight_height = DEFAULT_HEIGHT_WIGHT;
        if (null == lp) {
            lp = new LayoutParams(dip2Px(wight_height), dip2Px(wight_height),
                    Gravity.RIGHT | Gravity.TOP);
            setLayoutParams(lp);
        } else {
            wight_height = Math.min(lp.width, lp.height);
            if (wight_height <= 0) {
                wight_height = DEFAULT_HEIGHT_WIGHT;
            }
        }
        setBackground(wight_height, Color.parseColor("#d3321b"));
     ...
     *
     */
    public void setBackground(int dimeterDP, int badgeColor) {

        int radius = dip2Px(dimeterDP);
