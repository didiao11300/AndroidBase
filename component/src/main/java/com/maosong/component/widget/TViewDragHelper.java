...
/**
 * TViewDragHelper is a utility class for writing custom ViewGroups. It offers a number
 * of useful operations and state tracking for allowing a user to drag and reposition
 * views within their parent ViewGroup.
 */
public class TViewDragHelper {
    private static final String TAG = "TDrawer#TViewDragHelper";
    ...
    private static final int EDGE_SIZE = 270; // dp
    ...
    /**
     * A Callback is used as a communication channel with the TViewDragHelper back to the
     * parent view using it. <code>on*</code>methods are invoked on siginficant events and several
     * accessor methods are expected to provide the TViewDragHelper with more information
    ...
    public abstract static class Callback {
         ...
         * one of these methods, the TViewDragHelper will enter {@link #STATE_SETTLING}
         * and the view capture will not fully end until it comes to a complete stop.
         * If neither of these methods is invoked before <code>onViewReleased</code> returns,
         * the view will stop in place and the TViewDragHelper will return to
         * {@link #STATE_IDLE}.</p>
         *
         ...
         *
         * <p>TViewDragHelper may call this method multiple times for the same view even if
    ...
    /**
     * Factory method to create a new TViewDragHelper.
     *
     * @param forParent Parent view to monitor
     * @param cb        Callback to provide information and receive events
     * @return a new TViewDragHelper instance
     */
    public static TViewDragHelper create(@NonNull ViewGroup forParent, @NonNull Callback cb) {
        return new TViewDragHelper(forParent.getContext(), forParent, cb);
    }
     ...
     * @return a new TViewDragHelper instance
     */
    public static TViewDragHelper create(@NonNull ViewGroup forParent, float sensitivity,
                                         @NonNull Callback cb) {
        final TViewDragHelper helper = create(forParent, cb);
        helper.mTouchSlop = (int) (helper.mTouchSlop * (1 / sensitivity));
        return helper;
    }

    /**
     * Apps should use TViewDragHelper.create() to get a new instance.
     ...
     * @param forParent Parent view to monitor
     */
    private TViewDragHelper(@NonNull Context context, @NonNull ViewGroup forParent,
                            @NonNull Callback cb) {
        if (forParent == null) {
    ...
    public void captureChildView(@NonNull View childView, int activePointerId) {
        if (childView.getParent() != mParentView) {
            throw new IllegalArgumentException("captureChildView: parameter must be a descendant "
                    + "of the TViewDragHelper's tracked parent view (" + mParentView + ")");
        }
     ...
     * of the TViewDragHelper's knowledge).
     *
    ...
    public boolean shouldInterceptTouchEvent(@NonNull MotionEvent ev) {
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_POINTER_DOWN: {
                ...
                // A TViewDragHelper can only manipulate one view at a time.
                if (mDragState == STATE_IDLE) {
                    final int edgesTouched = mInitialEdgesTouched[pointerId];
                    if ((edgesTouched & mTrackingEdges) != 0) {
                        mCallback.onEdgeTouched(edgesTouched & mTrackingEdges, pointerId);
...
//                LogUtil.i(TAG, "shouldInterceptTouchEvent()...dragState:" + mDragState);
...
//                        LogUtil.i(TAG, "shouldInterceptTouchEvent()...oldLeft:" + oldLeft + " dx:" + dx + " targetLeft:" + targetLeft + " newLeft:" + newLeft + " hDragRange:" + hDragRange);
    ...
    public void processTouchEvent(@NonNull MotionEvent ev) {
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE: {
//                LogUtil.i(TAG, "processTouchEvent()...dragState:" + mDragState);
                if (mDragState == STATE_DRAGGING) {
                    // If pointer is invalid then skip the ACTION_MOVE.
    ...
    private void dragTo(int left, int top, int dx, int dy) {
        LogUtil.i(TAG, "dragTo()...from:" + left + " dx:" + dx);
        int clampedX = left;
        int clampedY = top;
    ...
    private boolean isValidPointerForActionMove(int pointerId) {
        if (!isPointerDown(pointerId)) {
            Log.e(TAG, "Ignoring pointerId=" + pointerId + " because ACTION_DOWN was not received "
                    ...
                    + " TViewDragHelper did not receive all the events in the event stream.");
            return false;
        }
        return true;
    }
